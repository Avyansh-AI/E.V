...
PANEL_2 = "#101116"
ACCENT = "#5cd3ff"
ACCENT_SOFT = "rgba(92, 211, 255,0.12)"
ACCENT_LINE = "rgba(92, 211, 255,0.22)"
TEXT = "#ffffff"
...
def _soft_button_style() -> str:
    return (
        ...
        f"QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(92, 211, 255,0.08); }}"
    )
...
class AddDeviceDialog(QDialog):
    ...
    def _build_platform_page(self) -> QWidget:
        ...
        for idx, platform in enumerate(self._platforms()):
            ...
            badge_style = {
                "atomberg": "background: rgba(53, 255, 117, 0.08); color: #3ddc97; border: 1px solid rgba(53, 255, 117, 0.2); border-radius: 18px;",
...
class _DeviceTile(ClickableFrame):
        ...
        if kind == "fan":
            ...
            for s in range(1, 6):
                ...
                if is_on and s <= current_speed:
                    seg.setStyleSheet("background: #5cd3ff; border-radius: 2.5px;")
                else:
    ...
    def set_active(self, active: bool):
        ...
        is_on = bool(self.device.get("is_on"))
        if is_on:
            self.power_btn.setStyleSheet("QPushButton { color: #3ddc97; background: rgba(53, 255, 117, 0.08); border: 1px solid rgba(53, 255, 117, 0.25); border-radius: 15px; } QPushButton:hover { background: rgba(53, 255, 117, 0.15); }")
        else:
...
class EVHomePage(QWidget):
    def __init__(self, parent=None):
        ...
        self.setObjectName("EVHomePageModern")
        self.setStyleSheet(f"QWidget#EVHomePageModern {{ background: {BG}; }} QScrollArea {{ background: transparent; border: none; }}")
        ...
        self._view_all_act_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: {ACCENT}; border: none; font-size: 11px; font-weight: bold; text-align: left; padding-left: 4px; }} QPushButton:hover {{ color: #ff6b7a; }}")
    ...
    def _build_header(self, lay: QVBoxLayout):
        ...
        self._voice_button.setStyleSheet(f"QPushButton {{ background: rgba(255, 255, 255, 0.02); color: {TEXT}; border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; font-size: 16px; }} QPushButton:hover {{ border: 1px solid {ACCENT}; background: rgba(92, 211, 255,0.08); }}")
    ...
    def _build_voice_card(self, lay: QVBoxLayout):
        ...
        self._mic_orb.setStyleSheet(f"QLabel {{ background: rgba(92, 211, 255,0.12); color: {TEXT}; border: 2px solid rgba(92, 211, 255,0.55); border-radius: 43px; }}")
        ...
        for _ in range(36):
            ...
            bar.setStyleSheet("background: rgba(92, 211, 255,0.20); border-radius: 2px;")
    ...
    def _build_my_devices(self, lay: QVBoxLayout):
        ...
        self._add_btn.setStyleSheet(f"QPushButton {{ background: transparent; color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; padding: 0 14px; }} QPushButton:hover {{ background: rgba(92, 211, 255,0.10); }}")
        ...
        self._empty_add_btn.setStyleSheet(f"QPushButton {{ background: rgba(92, 211, 255,0.12); color: {TEXT}; border: 1px solid {ACCENT}; border-radius: 12px; }} QPushButton:hover {{ background: rgba(92, 211, 255,0.18); }}")
    ...
    def _open_drawer(self, device: dict[str, Any]):
        ...
        self._drawer_meta.setText(f"""
            <div style='line-height: 140%;'>
                ...
                <span style='color: rgba(255,255,255,0.45); font-size: 10pt;'>Connection:</span> <span style='color: #3ddc97; font-weight: bold; font-size: 10pt;'>Local</span><br>
        ...
        is_on = bool(device.get("is_on"))
        state_color = "#3ddc97" if is_on else "rgba(255,255,255,0.4)"
    ...
    def _refresh(self):
        ...
        if devices:
            status_html = (
                ...
                "<span style='color: #3ddc97; font-size: 12px; font-weight: bold;'>●</span> "
            ...
            status_html = (
                "<div style='line-height: 1.3;'>"
                "<span style='color: rgba(255,255,255,0.4); font-size: 10px; font-weight: bold;'>SYSTEM</span><br/>"
                "<span style='color: #ffb648; font-size: 12px; font-weight: bold;'>●</span> "
    ...
    def _refresh_activity(self):
        ...
        for row in self._activity_items:
            ...
            badge_style = {
                "fan": "background: rgba(53, 255, 117, 0.08); color: #3ddc97; border: 1px solid rgba(53, 255, 117, 0.25); border-radius: 16px;",
    ...
    def _animate_voice(self):
        ...
        if hasattr(self, "_wave_bars") and self._wave_bars:
            for idx, bar in enumerate(self._wave_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(92, 211, 255,0.55)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.18)'}; border-radius: 2px;")
        if hasattr(self, "_voice_small_bars") and self._voice_small_bars:
            for idx, bar in enumerate(self._voice_small_bars):
                ...
                bar.setStyleSheet(f"background: {'rgba(92, 211, 255,0.35)' if self._voice_state in ('Listening', 'Executing') else 'rgba(255,255,255,0.15)'}; border-radius: 2px;")
        if hasattr(self, "_voice_cmd_lbl") and self._voice_cmd_lbl:
            self._voice_cmd_lbl.setText({"Idle": '"E.V."', "Listening": '"Turn bedroom fan to speed 4"', "Thinking": '"Understanding..."', "Executing": '"Applying command..."', "Completed": '"Done"'}.get(self._voice_state, '"E.V."'))
