...
class PluginManager:
    ...
    def register_assistant(self, ev_obj) -> None:
        self.ev = ev_obj
        for p in list(self.plugins):
            try:
                fn = getattr(p, "on_ev_created", None)
                if callable(fn):
                    fn(ev_obj)
            except Exception as exc:
                print(f"[Plugins] on_ev_created error: {exc}")
    ...
    def dispatch(self, hook: str, *args, **kwargs):
        ...
        for p in list(self.plugins):
            try:
                ...
                if callable(fn):
                    # call with ev if plugin expects it
                    try:
                        res = fn(*args, **kwargs, ev=self.ev)
                    except TypeError:
                        res = fn(*args, **kwargs)
                    if res is True:
                        return True
