...
STARTUP_LOG     = Path(os.environ.get("LOCALAPPDATA", str(BASE_DIR))) / "E.V.AI" / "startup.log"
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are E.V., a calm, direct, and professional AI assistant. "
...
def _gemini_text_reply(prompt: str) -> str:
    ...
    system_prompt = (
        "You are E.V., a concise, helpful desktop assistant. "
...
def _wakeword_detected(text: str) -> bool:
    ...
    if compact in phrases or any(p in compact for p in phrases):
        return True
    return any(word in {"ev", "hey", "hi", "hello"} for word in words)
...
def _update_memory_async(user_text: str, ev_text: str) -> None:
    ...
    try:
        api_key = _get_api_key()
        if not should_extract_memory(user_text, ev_text, api_key):
            return
        data = extract_memory(user_text, ev_text, api_key)
        if data:
            update_memory(data)
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "E.V. automatically infers the best visual style from the topic, searches for a matching online template when available, "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "theme": {
                    "type": "STRING",
                    "description": "Optional presentation theme or visual direction such as neon, corporate, luxury, academic, sunset, or creative. If omitted, E.V. infers the best style automatically."
                },
                "outline": {
                    "type": "STRING",
    ...
    },
    {
        ...
        "close the assistant, say goodbye, or stop E.V. "
...
class EVLive:
    ...
    def __init__(self, ui: EVUI, dashboard=None, dashboard_started: bool = False, enable_dashboard: bool = True):
        self.ui             = ui
    ...
    def _on_text_command(self, text: str, source: str = "local"):
        ...
        try:
            ...
            if routed_text_home != text:
                print(f"[E.V.] Redirection: '{text}' -> '{routed_text_home}'")
                text = routed_text_home
        except Exception as e:
            print(f"[E.V.] Redirection error: {e}")
        ...
        if _looks_like_screen_request(text):
            try:
                self.ui.update_task_workspace(
                    ...
                    output="E.V. is inspecting the screen for what you asked about.",
                    percent=40,
                )
            except Exception:
                pass
    ...
    def _handle_smart_home_command(self, text: str, source: str = "local") -> bool:
        ...
        try:
            ...
            self.ui.write_log(f"E.V.: {detail}")
            self.speak(detail)
    ...
    def _on_meeting_update(self, payload: dict):
        ...
        if answer:
            self.ui.write_log(f"E.V.: {answer}")
    ...
    def _handle_attention_response(self, text: str) -> bool:
        ...
        if kind == "message":
            ...
            if self._attention_matches(lower, ("hear", "read", "what is it", "tell me", "show it", "open it")):
                ...
                self.ui.write_log(f"E.V.: {preview}")
    ...
    def _fallback_reply(self, text: str, memory_ctx: str = ""):
        try:
            ...
            try:
                self.ui.update_task_workspace(
                    status="Thinking",
                    output="E.V. is drafting a direct reply.",
                    percent=35,
                )
            except Exception:
                pass
            ...
            if gemini_first:
                try:
                    ...
                    print(f"[E.V.] ⚠️ Gemini fallback failed: {e}")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "Wake-word mode: if the microphone is muted, still listen for the words 'E.V.', 'hey', 'hi', and 'hello'. "
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[E.V.] 📤 {name} → {str(result)[:80]}")
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                ev_speaking = self._is_speaking
            if self._phone_active:
                return
            if not ev_speaking and (not self.ui.muted or getattr(self.ui, "_wakeword_listening", False)):
                data = indata.tobytes()
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"E.V.: {full_out}")
                            out_buf = []
    ...
    async def run(self):
        ...
        while True:
                ...
                try:
                    async with asyncio.TaskGroup() as tg:
                        ...
                        try:
                            ...
                            pass
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: E.V. online.")
            ...
            self.set_speaking(False)
            self.ui.set_state("LISTENING")
            print("[E.V.] 🔄 Reconnecting in 5s...")
            await asyncio.sleep(5)

def main():
    _startup_log("main entered")
    _ensure_desktop_shortcut()
    ui = EVUI(str(BASE_DIR / "assets" / "ev_logo.png"), show_immediately=True)
    dashboard = None
    ...
    if DashboardServer is not None and not dashboard_enabled:
        ...
        try:
            ui.write_log("SYS: Mobile Connect is already running in another E.V. instance.")
        except Exception:
            pass
    ...
    def runner():
        ...
        try:
            if plugin_manager is not None:
                ev.plugin_manager = plugin_manager
                plugin_manager.register_assistant(ev)
                # allow plugins to run a startup hook
                try:
                    plugin_manager.dispatch("on_startup", ev)
                except Exception:
                    pass
        except Exception:
            pass
